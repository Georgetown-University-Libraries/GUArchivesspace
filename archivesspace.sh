...
if [ "$ARCHIVESSPACE_USER" = "" ]; then
    ARCHIVESSPACE_USER="archives"
fi

# 2018-11-09, mm3941
# custom log location
if [ "$ARCHIVESSPACE_LOGS" = "" ]; then
    ARCHIVESSPACE_LOGS="/var/log/archivesspace/archivesspace.out"
fi

export JAVA_OPTS="-Darchivesspace-daemon=yes $JAVA_OPTS -Djava.security.egd=file:/dev/./urandom"

# 2018-11-08, brockjd
# enable JMX for performance monitoring
export JAVA_OPTS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=7199 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false $JAVA_OPTS"

# 2018-11-09, mm3941
# increase MaxPermSize
export ASPACE_JAVA_MAXPERMSIZE="-XX:MaxPermSize=512m"
...
if [ "$ASPACE_JAVA_MAXPERMSIZE" = "" ]; then
    ASPACE_JAVA_MAXPERMSIZE="-XX:MaxPermSize=512m"
fi
